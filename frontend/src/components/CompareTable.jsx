...
export function CompareTable({
  ...
  const isSent = (id) => {
    ...
    return (
      ...
      sentProfileIds.some((sid) => String(sid) === idStr)
    );
  };
      ...
      <div className="bg-white rounded-[20px] p-6 satfera-shadow">
        ...
        <div className="flex gap-4 overflow-x-auto pb-2">
          {slots.map((profile, index) => (
            <div
              key={profile?.id || profile?.userId || `empty-${index}`}
              className="flex-shrink-0 relative overflow-visible pt-3 px-3"
            >
              {profile ? (
                <>
                  ...
                  <X
                    onClick={(e) => {
                      e.stopPropagation();
                      const pid =
                        profile?.id || profile?.userId || profile?._id;
                      if (!pid) {
                        console.warn(
                          "onRemove called but profile id is missing",
                          profile
                        );
                        return;
                      }
                  ...
                  <div className="w-24">
                    {resolveField(profile, "image") ? (
                      <div
                        className="w-24 h-24 rounded-[12px] overflow-visible"
                        style={{ position: "relative" }}
                      >
                        <img
                          src={resolveField(profile, "image")}
                          alt={capitalize(resolveField(profile, "name")) || ""}
                          className="w-full h-full object-cover"
                        />
                      </div>
                    ) : (
                      <div className="w-24 h-24 rounded-[12px] bg-gray-100 flex items-center justify-center text-xs text-muted-foreground">
                        No image
                      </div>
                    )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-border-subtle bg-white">
                <th
                  className="text-left p-4 text-sm text-muted-foreground"
                ...
                {slots.map((profile, index) => (
                  <th
                    key={profile?.id || `empty-header-${index}`}
                    ...
                    {profile ? (
                      <div className="flex flex-col items-center gap-2">
                        {resolveField(profile, "image") ? (
                          ...
                          <div className="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center text-xs text-muted-foreground">
                            No image
                          </div>
                        )}
                        <div>
            ...
            <tbody>
              {rows.map((row, index) => (
                ...
                >
                  ...
                  {slots.map((profile, i) => (
                    <td
                      key={profile?.id || `empty-${row.key}-${i}`}
          ...
          <div className="border-t border-border-subtle bg-white">
            <div className="flex">
              ...
              {slots.map((profile, index) => (
                <div
                  key={profile?.id || `empty-action-${index}`}
                  className="flex-1 p-4 flex flex-col gap-2 min-w-[200px]"
                >
                  {profile ? (
                    <>
                      <Button
                      ...
                      <Button
                        ...
                        disabled={isSent(getProfileId(profile))}
                        className={
                          isSent(getProfileId(profile))
