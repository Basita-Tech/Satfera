      ...
      <DialogContent showClose={false} className="sm:max-w-md max-w-[82vw] sm:my-12 my-10 sm:mx-6 mx-6 rounded-[18px] bg-white shadow-2xl border border-border-subtle max-h-[80vh] overflow-hidden">
        ...
        <div className="space-y-4 px-4 sm:px-6 py-4">
          <div className="space-y-2">
            ...
            <Input id="customId" placeholder="Enter User ID (e.g., USR12345)" value={customId} onChange={e => setCustomId(e.target.value)} className="rounded-[12px] border-border-subtle h-12" onKeyDown={e => {
        ...
        <div className="flex flex-col sm:flex-row gap-3 px-4 sm:px-6 pb-4">
          <Button variant="outline" onClick={() => handleOpenChange(false)} disabled={isBlocking} className="flex-1 rounded-[12px] border-border-subtle h-11 sm:w-auto w-full">
            Cancel
          </Button>
          <Button variant="outline" onClick={handleBlock} disabled={isBlocking || !customId.trim()} className="flex-1 bg-white border-2 border-red-500 text-red-500 hover:bg-red-50 hover:text-red-600 rounded-[12px] h-11 sm:w-auto w-full">
            {isBlocking ? <>
