...
export function RequestsSent({
  ...
  return <div className="space-y-6">
      ...
      {}
      {filteredProfiles.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {filteredProfiles.map(profile => <ProfileCard key={profile.id} {...profile} variant="sent" onView={onViewProfile} onWithdraw={handleWithdrawClick} onAddToCompare={onAddToCompare} onRemoveCompare={onRemoveCompare} onChat={() => setPremiumModal(true)} isInCompare={Array.isArray(compareProfiles) ? compareProfiles.map(String).includes(String(profile.id || profile._id || profile.userId)) : false} isShortlisted={Array.isArray(shortlistedIds) ? shortlistedIds.some(sid => String(sid) === String(profile.id)) : false} onToggleShortlist={onToggleShortlist} />)}
          ...
          <div className="flex flex-col items-center gap-4 max-w-md mx-auto">
            ...
            <p className="text-muted-foreground m-0">
              {activeTab === "all" ? "You haven't sent any requests yet. Browse profiles to find your perfect match!" : `No ${activeTab} requests at the moment.`}
            </p>
          </div>
        </div>}

      {}
      <WithdrawModal open={withdrawModal.open} onOpenChange={open => setWithdrawModal({
      open,
      profile: null
    })} profileName={withdrawModal.profile?.name || ""} onConfirm={handleConfirmWithdraw} />
