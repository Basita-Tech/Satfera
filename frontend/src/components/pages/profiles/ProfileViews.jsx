...
export function ProfileViews({ 
  ...
  // Calculate this week's views from the views array (views within last 7 days)
  const calculateWeeklyViews = () => {
    ...
    return views.filter(view => {
      ...
      // If viewedAt is already formatted (like "2h ago"), count it as recent
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-8 py-4 md:py-6 lg:py-8 space-y-4 md:space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p
                className="text-[2rem] text-gold m-0"
      ...
      {views.length > 0 && (
        <div className="space-y-4">
          {views.map((view) => (
            <div
              key={view.id}
              className="bg-white rounded-[20px] p-6 satfera-shadow hover:shadow-lg transition-shadow"
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
                ...
                <img
                  src={view.image}
                  alt={view.name}
                  className="w-16 h-16 sm:w-20 sm:h-20 rounded-[16px] object-cover flex-shrink-0 mx-auto sm:mx-0"
                />

                {/* Info */}
                <div className="flex-1 text-center sm:text-left w-full sm:w-auto">
                  <div className="flex flex-col sm:flex-row items-center sm:items-center gap-2 sm:gap-3 mb-1">
                    <h4 className="m-0" style={{ fontSize: '1.125rem', fontWeight: 600 }}>
                      {view.name}, {view.age}
                    </h4>

                    {view.compatibility && view.compatibility > 0 && (
                      <span 
                        className="text-sm px-3 py-1 bg-gold/10 text-gold rounded-full font-medium"
                ...
                {/* Button */}
                <button 
                  onClick={() => handleViewProfile(view.id)}
                  className="w-full sm:w-auto px-4 sm:px-6 py-2 bg-gold hover:bg-gold/90 text-white rounded-[12px] transition-all flex-shrink-0 text-sm sm:text-base"
                >
                  View Profile
                </button>
              </div>
      ...
      {/* Pagination - Debug */}
      {console.log('Pagination Debug:', { pagination, onPageChange: !!onPageChange })}
      
      {/* Pagination */}
      {pagination && pagination.totalPages > 1 && onPageChange && views.length > 0 && (
        <div className="flex justify-center items-center gap-4 mt-6 bg-white rounded-[20px] p-4 satfera-shadow">
          <button
            onClick={() => {
            ...
            className="px-4 py-2 bg-gold hover:bg-gold/90 text-white rounded-[12px] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
