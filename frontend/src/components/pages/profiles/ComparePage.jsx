...
export function ComparePage({
  ...
  useEffect(() => {
    ...
    const doResolve = async () => {
      ...
      for (const sp of selectedProfiles) {
        ...
        if (found) results.push(found);else toFetch.push(sp);
      }
      if (toFetch.length) {
        try {
          const {
  ...
  const compareProfileIds = Array.isArray(selectedProfiles) ? selectedProfiles.map(p => typeof p === "object" ? p?.id || p?.userId || p?._id : p).filter(Boolean) : [];
  const allProfiles = [...(Array.isArray(profiles) ? profiles : []), ...fetchedProfiles];
  const shortlistedProfiles = Array.isArray(allProfiles) && Array.isArray(shortlistedIds) ? allProfiles.filter(p => {
  ...
  return <div className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-8 py-4 md:py-6 lg:py-8 space-y-4 md:space-y-6">
      ...
      {}
      <CompareTable profiles={resolvedSelectedProfiles} onRemove={onRemoveFromCompare} onViewProfile={onViewProfile} onSendRequest={onSendRequest} onAddProfile={() => setAddModalOpen(true)} shortlistedIds={shortlistedIds} onToggleShortlist={onToggleShortlist} sentProfileIds={sentProfileIds} />

      {}
      <AddProfileModal open={addModalOpen} onOpenChange={setAddModalOpen} profiles={allProfiles} shortlistedProfiles={shortlistedProfiles} onAddToCompare={handleAddToCompare} onRemoveCompare={onRemoveFromCompare} compareProfileIds={compareProfileIds} shortlistedIds={shortlistedIds} onToggleShortlist={onToggleShortlist} />
    </div>;
}