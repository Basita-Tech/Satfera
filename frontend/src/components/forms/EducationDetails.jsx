      ...
      <div className="bg-[#FBFAF7] shadow-2xl rounded-3xl w-full max-w-xl p-4 sm:p-8 border-t-4 border-[#F9F7F5] transition-transform duration-300">
        ...
        <form onSubmit={handleNext} className="space-y-6">
          ...
          <div className="flex flex-col">
            ...
            <CreatableSelect key={formData.highestEducation || 'no-qualification'} isClearable options={educationOptionsFormatted} value={formData.fieldOfStudy} onChange={(newValue, actionMeta) => {
            handleFieldOfStudyChange(newValue);
            if (actionMeta.action === "select-option") {
              document.activeElement.blur();
            }
          }} placeholder={formData.highestEducation ? "Select or type field of study" : "Please select highest qualification first"} isDisabled={!formData.highestEducation} classNamePrefix="react-select" components={{
            IndicatorSeparator: () => null
          }} styles={{
            control: (base, state) => {
              let borderColor = "#d1d5db";
              if (errors.fieldOfStudy) borderColor = "red";else if (formData.fieldOfStudy && formData.fieldOfStudy.value) borderColor = "#D4A052";else if (state.isFocused) borderColor = "#E4C48A";
              return {
                ...base,
                minHeight: "3rem",
                borderRadius: "0.5rem",
          ...
          }} menuPlacement="top" menuPosition="absolute" />
