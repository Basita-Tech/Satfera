...
const fetchFullProfile = async (profileId) => {
  try {
    console.log('🔍 Fetching full profile data for ID:', profileId);
    ...
    if (response?.success && response?.data) {
      console.log('✅ Full profile data retrieved:', response.data);
      console.log('✅ Profile structure:', {
        hasPersonal: !!response.data.personal,
        hasEducation: !!response.data.education,
        hasProfessional: !!response.data.professional,
        hasFamily: !!response.data.family,
        hasHealthAndLifestyle: !!response.data.healthAndLifestyle,
        personalKeys: response.data.personal ? Object.keys(response.data.personal) : [],
        educationKeys: response.data.education ? Object.keys(response.data.education) : [],
        professionalKeys: response.data.professional ? Object.keys(response.data.professional) : [],
      });
      return response.data;
    }
    
    console.warn('⚠️ API response did not contain profile data');
    return null;
  } catch (error) {
...
export const generateProfilePDF = async (profile) => {
  try {
    console.log('📄 Starting PDF generation with profile data:', profile);
    ...
    console.log('🔑 ID sources checked:', {
    ...
    if (profileId) {
      ...
      if (fetchedProfile) {
        console.log('✅ Successfully fetched full profile data');
        console.log('✅ Fetched profile has:', {
          personal: !!fetchedProfile.personal,
          education: !!fetchedProfile.education,
          professional: !!fetchedProfile.professional,
          family: !!fetchedProfile.family,
          healthAndLifestyle: !!fetchedProfile.healthAndLifestyle
        });
    ...
    console.log('📊 Final profile data for PDF generation');
    ...
    doc.text('APPROVED MATCH PROFILE', pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 10;
    ...
    console.log('📝 Generated full name for PDF:', fullName);
    ...
    if (dob) {
      const birthDate = new Date(dob);
      const formattedDate = birthDate.toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' });
    ...
    doc.setFontSize(8);
    doc.setTextColor(100, 100, 100);
    doc.text('Generated from Satfera - Approved Match', pageWidth / 2, footerY, { align: 'center' });
