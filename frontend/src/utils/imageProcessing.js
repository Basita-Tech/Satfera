...
// Auto-adjusts uploaded photos by type (closer, family, personal, other)

const TARGETS = {
  closer: { w: 600, h: 600, mime: 'image/jpeg' }, 
  family: { w: 1200, h: 900, mime: 'image/jpeg' }, 
  personal: { w: 900, h: 1200, mime: 'image/jpeg' }, 
  other: { w: 1200, h: 1200, mime: 'image/jpeg' }, 
};
...
async function canvasToFile(canvas, mime = 'image/jpeg', baseName = 'image', maxBytes = 2 * 1024 * 1024) {
  ...
  // Reduce quality if exceeds maxBytes
...
export async function processAndAdjustImage(file, category = 'other') {
  try {
    ...
    const { sx, sy, sw, sh } = computeCoverCrop(img.naturalWidth || img.width, img.naturalHeight || img.height, target.w, target.h);
...
export default { processAndAdjustImage };
