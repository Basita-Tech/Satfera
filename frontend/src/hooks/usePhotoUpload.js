...
const usePhotoUpload = () => {
  ...
  const setupNetworkMonitoring = useCallback(() => {
    const handleOnline = () => {
      ...
      console.log("[PhotoUpload] Network connection restored");
    };
    const handleOffline = () => {
      isOnlineRef.current = false;
      console.log("[PhotoUpload] Network connection lost");
    };
  ...
  const waitForNetwork = useCallback(async (maxWaitTime = 10000) => {
    ...
    console.log("[PhotoUpload] Waiting for network connection...");
    ...
    async (
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          console.log(
            `[PhotoUpload] Successfully uploaded ${photoKey}:`,
            response.data
          );
          ...
          if (
            ...
            console.log(`[PhotoUpload] Client error ${status}, not retrying`);
            break;
          }

          if (attempt < maxRetries) {
            const backoffDelay = Math.pow(2, attempt - 1) * 1000;
            console.log(`[PhotoUpload] Retrying in ${backoffDelay}ms...`);
      ...
      try {
        for (let i = 0; i < photosToUpload.length; i++) {
          ...
          if (abortControllerRef.current.signal.aborted) {
            console.log("[PhotoUpload] Upload aborted by user");
            break;
          }
          ...
          if (result.success) {
            ...
            setUploadState((prev) => ({
              ...
              errors: [...prev.errors, { key, error: result.error }],
          ...
          if (i < photosToUpload.length - 1) {
            console.log("[PhotoUpload] Waiting 800ms before next upload...");
        ...
        setUploadState((prev) => ({
          ...
          isUploading: false,
          currentPhoto: null,
          errors: [...prev.errors, { key: "fatal", error: error.message }],
        }));
        ...
        return {
          success: false,
          results,
          errors: [...errors, { key: "fatal", error: error.message }],
          successCount,
          failedCount,
          totalPhotos: photosToUpload.length,
        };
  ...
  const abortUpload = useCallback(() => {
    if (abortControllerRef.current) {
      ...
      console.log("[PhotoUpload] Upload aborted");
    }
  }, []);
  ...
  const retryFailedUploads = useCallback(
    async (photosToUpload) => {
      ...
      if (failedPhotos.length === 0) {
        console.log("[PhotoUpload] No failed uploads to retry");
        return { success: true, results: [], errors: [] };
      }

      console.log(
        `[PhotoUpload] Retrying ${failedPhotos.length} failed uploads...`
      );
